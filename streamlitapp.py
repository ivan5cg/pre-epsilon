...
# 2. Función para obtener datos de yfinance eficientemente
@st.cache_data(ttl=60)  # Caché de 60 segundos para no saturar la API
def obtener_datos(tickers):
    data_list = []
    
    # Descargamos datos de los últimos 2 días para calcular la variación
    # Usamos group_by='ticker' para facilitar el manejo de múltiples acciones
    try:
        df = yf.download(tickers, period="5d", group_by='ticker', progress=False)
        
        for ticker in tickers:
            # Extraer datos específicos del ticker
            if len(tickers) > 1:
                hist = df[ticker]
            else:
                hist = df # Si es solo uno, la estructura es diferente
            
            # Verificamos que haya datos suficientes
            if len(hist) >= 2:
                ...
                # Determinamos color y símbolo
                color = "#4caf50" if cambio >= 0 else "#ff5252" # Verde / Rojo
...
# 3. Función para generar el HTML/CSS del Ticker
def mostrar_ticker(datos):
    # Creamos los elementos HTML para cada acción
    items_html = ""
    for item in datos:
        items_html += f"""
        <div class="ticker-item">
            <span class="ticker-symbol">{item['ticker']}</span>
            <span class="ticker-price">${item['precio']}</span>
            <span class="ticker-change" style="color: {item['color']};">
                {item['simbolo']} {item['porcentaje']}
            </span>
        </div>
        """

    # CSS para la animación y el modo oscuro
    estilo_css = """
    <style>
        .ticker-wrap {
            width: 100%;
            overflow: hidden;
            background-color: #0e1117; /* Fondo oscuro de Streamlit por defecto */
        ...
        .ticker-move {
            ...
            white-space: nowrap;
            padding-right: 100%; /* Empieza fuera de la pantalla */
            animation: ticker 30s linear infinite; /* Velocidad de la animación */
        }
    ...
    # HTML final combinando contenedor y estilos
    html_completo = f"""
    {estilo_css}
...
mis_acciones = ["AAPL", "GOOGL", "MSFT", "TSLA", "AMZN", "NVDA", "BTC-USD", "ETH-USD"]

# Obtener datos
datos_mercado = obtener_datos(mis_acciones)

# Mostrar el letrero si hay datos
if datos_mercado:
    mostrar_ticker(datos_mercado)
else:
